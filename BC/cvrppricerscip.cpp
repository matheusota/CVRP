        ...
        for(EdgeIt e(cvrp.g); e != INVALID; ++e){
            ...
            for (list<ConsInfo*>::iterator it = consList.begin(); it != consList.end(); ++it){
                ...
                if((*it)->cons != NULL){
                    SCIP_CONS *transfCons;
                    SCIPgetTransformedCons(scip, (*it)->cons, &transfCons);
                    double aux = SCIPgetDualsolLinear(scip, (*it)->cons);
                    if(aux != 0){
                        r -= (*it)->coef * aux;
                    }
                }
                else{
                    double aux = SCIProwGetDualsol((*it)->row);
                    if(aux != 0){
                        r -= (*it)->coef *  aux;
                    }
                }
    ...
    for(QRit it = qroutes.begin(); it != qroutes.end(); ++it){
        for(EdgeIt e(cvrp.g); e != INVALID; ++e){
            if((*it)->edgeCoefs[cvrp.g.id(e)] > 0){
                SCIP_VAR *transfVar;
                SCIP_CALL(SCIPgetTransformedVar(scip, (*it)->var, &transfVar));
                SCIP_CALL(SCIPaddCoefLinear(scip, translateMap[e], transfVar, (*it)->edgeCoefs[cvrp.g.id(e)]));
            }
        }
    }
