...
int main(int argc, char *argv[])
{
    ...
    l.shouldPrice = params.shouldPrice;
    l.testing = false;
    l.ncolumns = 0;
    l.nrows = 0;
    clock_t before = clock();
    ...
    if(params.verbosity == GRAPH){
        ...
        //write some data to a separate file
        ofstream data;
        data.open ("data/output_data.txt", std::ios_base::app);

        std::size_t found = params.inputFile.find_last_of("/");
        string tmp = params.inputFile.substr(found+1);
        string instanceName = params.inputFile.substr(found+1, tmp.size() - 4);

        data << instanceName << " & " << l.ncolumns << " & " << l.nrows << " & " << ts.cost << " & " << elapsedTime << " \\\\ \\hline\n";
        data.close();
    }
    cout << endl;
              ...
              std::unordered_map<int,Node> &int2node)
{
    ...
    Node u,v;
    std::unordered_map<int,Node> test;
...
void solutionAsGraphical(CVRPInstance &instance, CVRPSolution  &sol, string inputFile)
{
    ...
    string tmp = inputFile.substr(found+1);
    string instanceName = inputFile.substr(found+1, tmp.size() - 4);
